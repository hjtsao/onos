...
final class P4RuntimeClientImpl extends AbstractGrpcClient implements P4RuntimeClient {
    ...
    // P4MT: Used by this client for write requests.
    private Uint128 clientElectionId = Uint128.newBuilder().setLow(1).build();

    // P4MT: Role ID
    private P4RuntimeOuterClass.Role role;
    ...
    P4RuntimeClientImpl(P4RuntimeClientKey clientKey, ManagedChannel channel,
        ...
        // P4MT
        configService = DefaultServiceDirectory.getService(NetworkConfigService.class);
        BasicDeviceConfig cfg = configService.getConfig(deviceId, BasicDeviceConfig.class);
        role = P4RuntimeOuterClass.Role.newBuilder()
                .setId(cfg.roleId())
                .build();
        log.info("P4MT: RoleID {}", cfg.roleId());
    ...
    private boolean sendMasterArbitrationUpdate(boolean asMaster) {
        ...
        // Code looks very weird due to support of P4MT as a slave controller
        if(role.getId() != 0) {
            BigInteger bigInteger = BigInteger.ONE;
            final Uint128 electionId = bigIntegerToUint128(bigInteger);
            StreamMessageRequest streamMessageRequest = StreamMessageRequest.newBuilder()
                    .setArbitration(
                            MasterArbitrationUpdate.newBuilder()
                                    ...
                                    .build())
                    .build();
            streamChannelManager.send(streamMessageRequest);
            log.info("Sending arbitration update to {}... electionId={}... roleId={}",
                     deviceId, streamMessageRequest.getArbitration().getElectionId(),
                     streamMessageRequest.getArbitration().getRole().getId());
        } else {
    ...
    private boolean doIsPipelineConfigSet(PiPipeconf pipeconf, ByteBuffer deviceData) {
        // P4MT: Keep getting 'cookie' field unset from server, skip this now
        if(role.getId() != 0) {
            return true;
        }
    ...
    private boolean doSetPipelineConfig(PiPipeconf pipeconf, ByteBuffer deviceData) {
        ...
        if(role.getId() != 0) {
            log.info("P4MT slave controller: skip 'doSetPipelineConfig()'");
            return true;
        } else {
    ...
    private List<PiTableEntry> doDumpTables(
        ...
        log.info("Retrieved {} entries from {} tables on {}...",
    ...
    private void doArbitrationResponse(MasterArbitrationUpdate msg) {
        ...
        log.info("Received arbitration update from {}: isMaster={}, electionId={}, roleId={}",
                  deviceId, isMaster, uint128ToBigInteger(msg.getElectionId()), msg.getRole().getId());
        ...
        if(role.getId() != 0) {
            ...
            // P4MT: To spoof ONOS that we are master
